...
> [!CAUTION]
> **v1.0.1 免责声明：** 本工具刚完成 v1.0.1 的初始开发和验证，仍可能存在尚未发现的 bug。错误的 matching、mapping、update 或 soft-delete 可能导致 Snipe-IT inventory 出现重复记录、错误匹配、关联错误或资产缺失等混乱。使用者应自行承担使用风险。在首次执行 `Sync Now`、启用 Schedule，以及每次升级版本或修改 mapping 前，强烈建议先使用 Snipe-IT 的 Backup 功能创建完整、可恢复的 snapshot，并确认该备份能够正常恢复。务必先运行并检查 `Preview`；Preview 不能替代 backup。

...
Step 4 的目标是实现 Atera 设备数据读取，交付 `AteraClient`、Atera agent DTO、完整设备集合读取、auth failure handling、retryable failure handling 和 mocked API tests。
...
由于当前没有独立的 Atera 测试环境，任何真实 Atera API call 都必须非常谨慎。若官方文档无法确认 API 用法，后续 agent 必须先停止实现并向项目 owner 说明不确定点；只有在 owner 明确同意后，才可以在测试区用最小范围的 Python 探针脚本验证 API 行为。
...
成功拉取后，每条可用 agent record 应转换为内部 `AteraAgentDto`。DTO 字段代表系统后续模块需要的 Atera 设备原始信息；具体字段如何从 Atera wire response 映射，必须以后续官方 API 文档验证结果为准。

customer 不应作为本模块的独立主资源输出。如果 Atera agent/device response 中包含 customer id、customer name、site、company 或类似归属信息，本模块可以把这些信息作为 `AteraAgentDto` 的属性保留，供 Reconstruction Module 后续使用。
...
summary 中的 agent count 必须与输出 `AteraAgentDto` 集合数量一致。completion timestamp 应由模块在 pull 成功完成时生成。
...
- 读取或写入配置文件
- 在未经 owner 明确同意时调用真实 Atera API 做探索性测试

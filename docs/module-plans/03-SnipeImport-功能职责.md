...
本模块根据 import batch 中的 company、manufacturer、category、model、asset 字段查询 Snipe-IT，并决定每条 asset 应该被创建、更新、跳过或标记失败；同时将完整 Snipe-IT hardware snapshot 与本次 Atera asset tags 对比，删除已经不在 Atera inventory 中的自动同步资产。
...
- `Preview Changes` 本身必须使用 dry-run，不执行真实 `POST` / `PATCH` / `DELETE`
...
- 每个 CSV 必须包含一列 `Operation`，用于说明本行是 `Add`、`Modify`、`Blocked` 还是 `Delete`
- 第一版当前会产生的 CSV 表：
  - `snipeit-assets-plan.csv`
  - `snipeit-companies-plan.csv`
  - `snipeit-categories-plan.csv`
  - `snipeit-models-plan.csv`
- `snipeit-assets-plan.csv` 必须为每个过期自动同步资产输出 `Operation = Delete`、Snipe-IT asset id、asset tag、设备名与 `ChangeReasons = MissingFromAtera`，让 operator 在 Preview 中先看到真实删除范围
...
- 只有 trim 后以 `ATERA-` 开头（ordinal ignore-case）的 Snipe-IT Hardware Asset 才属于自动同步资产；该约定是删除所有权边界，人工资产不参与删除比较
- 本次 batch 中所有 trim 后的完整 asset tag 组成当前 Atera inventory identity set；自动同步资产的完整 asset tag 不在该集合时，才计划 `DELETE /hardware/{id}`
- 删除比较必须基于一次完整、合法的 hardware snapshot；snapshot 缺页、malformed 或读取失败时 fail closed，不得执行任何 asset create/update/delete
- dry-run 只产生 planned `Delete/Asset` action 和 Preview CSV，不发送 `DELETE`；真实运行仅在对应 delete response 通过 HTTP 与 Snipe-IT business-success 检查后增加 deleted count 和 executed action
- 每次删除成功必须写结构化 Information log；删除失败必须写结构化 Warning log。两者均包含 Snipe-IT asset id、asset tag、设备名、从 `ATERA-` 后缀解析的 Atera Agent ID、固定删除原因和结果，但不得包含 API token、raw response body 或完整 notes
- structured action/history 的 deleted item 必须保留 asset id、asset tag、设备名、Atera Agent ID 与删除原因，方便管理员从同步历史定位被删设备
...
- 单个 `DELETE /hardware/{id}` 返回 HTTP 或 business error；该失败必须记录为对应 asset 的 `ImportFailure` 并继续处理其它删除候选
...
- 写 status file
- 发送 email 或 webhook
- 归档、checkout、checkin Snipe-IT asset
- 创建 category
- 创建 manufacturer
...
- checkout/checkin 行为
- disappeared devices archive policy（当前行为是 soft-delete，不是 archive）
...
## 10.8 Asset 真正变更预检与 no-op 职责

- `snipeit-assets-plan.csv` 是本次运行的实际写入/阻塞计划，不是“已匹配资产清单”。它只允许包含将创建的 `Add`、至少一个受管字段真正不同的 `Modify`，以及无法继续的 `Blocked`；完全一致的 existing asset 不得出现在该 CSV。
- existing asset 命中后，Snipe Import 必须比较实际 hardware payload 管理的业务字段：`asset_tag`、`status_id`、`model_id`、`company_id`、`name`、可靠 serial、配置的 MAC custom field，以及去除本工具审计时间行后的业务 notes。
- asset diff 必须使用同一 run 已解析的 company/model ids、同一个 MAC payload selection 规则和同一套文本/identity normalization；不能为 Preview 和真实 Sync 分别实现两套判断。
- `First added by Atera-SnipeIT Sync Tool at ...` 与 `Auto Synced from Atera at ...` 是工具管理的审计行。已有 first-added 行必须继续保留，但时间行本身不得成为 `Modify` 原因；否则每次运行都会制造虚假更新。
- snapshot 中缺少某个需要比较的目标字段时必须 fail safe：把该字段列为变更原因并保留 `Modify`，不得因无法证明相等而跳过必要写入。
- `Modify` CSV row 必须输出稳定顺序的 `ChangeReasons`，让 operator 能看到触发更新的字段；`Add` 使用 `Create`，`Blocked` 使用空值并继续依赖现有 conflict/failure columns。
- 完全一致的 existing asset 在 dry-run 与真实 Sync 都计入 `SkippedAssets`，产生结构化 `Skip/Asset` action，但不得发送 `PATCH /hardware/{id}`。dry-run 的 skip action 为 planned，真实 Sync 的 skip action表示已完成 no-op 判定，但不计入已执行 HTTP write 数。
- 真实 Sync 与 Preview 必须对同一 snapshot 产生相同的 Add/Modify/Skip 决策；区别仅在 Preview 不执行 mutation。
- 每次真实 Sync 成功更新一个 asset 后，运行日志必须写明该 asset 的 Snipe-IT id、asset tag 与稳定顺序的 changed-field labels。相同 details 还必须进入 per-record progress 和 executed action/history，确保手动与排程运行都能审计“改了什么”；不得把 notes 正文、API token 或其他 secret 写入日志。

## 10.9 Company 分页与内部职责边界

- Company snapshot 必须按官方 `GET /companies?limit=500&offset={offset}` 契约读取全部页面；首个响应的 `total` 是本次 snapshot 的稳定基线。
- 后续页面的 `total` 改变、累计达到 `total` 前返回空页、页数超过安全上限，或最终累计数与 `total` 不一致时，必须以 `SnipeImport.IncompleteCompanySnapshot` fail closed，任何 reference/asset mutation 都不得开始。
- HTTP 鉴权、URI、重试和 Snipe-IT `200 + status:error` 解释属于 API transport；分页和 snapshot 完整性属于 snapshot loader；验证、匹配和 action 生成属于 planner；reference/model/asset mutation 属于 executor。
- `SnipeImporter` 只保留 `ISnipeImporter` facade 和一次 run 的阶段编排。拆分不得改变公开构造函数、endpoint、payload、retry、cancellation、progress、warning、failure 或 audit 语义。
- 独立 `SnipeModelCategoryNormalizer` 不再作为第二套生产实现存在；其仍有价值的规则和测试必须迁移到普通 Preview/Sync 共用的 planner/executor。

成功条件：任意公司数量都能读取完整 snapshot；不稳定或不完整分页不会产生写入；重构前后的 mocked HTTP trace 与结果保持兼容。

## 10.10 Atera-managed Asset 对比删除与管理员审计职责

- 所有权只由 Snipe-IT hardware `asset_tag` 判断。trim 后以 `ATERA-` 开头（ordinal ignore-case）的资产视为自动同步资产；其它 asset tags 一律视为人工/外部资产并保留，即使名称、serial、MAC 或 notes 与 Atera 相似。
- 当前 Atera identity set 来自本次 `SnipeImportBatch.Assets` 的完整 normalized asset tags，而不是成功匹配或成功写入的子集。单条 source record 后续 reference/matching 失败时，其 `ATERA-{SourceId}` 仍必须保护同 tag 的现有资产，避免把“本次无法更新”误判为“已从 Atera 消失”。
- importer 在 Atera batch 为空时仍必须完整读取一次 hardware snapshot；非空 batch 至少有一个可匹配 record 时，沿用匹配所需的同一 snapshot。若非空 batch 的所有 records 已在 validation/reference 阶段阻塞，则 fail closed、不读取额外 hardware snapshot，也不计划删除。snapshot 中每个 Atera-managed asset 若完整 normalized tag 不在当前 identity set，则生成一个 deterministic delete plan；候选按 Snipe-IT asset id 排序。
- 任意 source validation、reference、matching 或 duplicate-target failure 都会关闭本次 run 的全部 stale deletes；已有 create/update partial-processing 语义可以继续，但无法证明 source inventory 完整可用时不得删除。
- Delete plan 必须与 Add/Modify/Blocked 一起进入 `snipeit-assets-plan.csv`。dry-run 增加 `DeletedAssets` planned count 和 `Delete/Asset` planned action，但不发送 mutation。
- 真实执行顺序固定为 reference mutations → asset create/update/no-op → stale asset deletes。reference gate 或 preflight CSV gate 失败时不得开始 delete；单个 asset create/update failure 不隐藏已经安全规划的其它 stale candidates。
- 真实删除使用官方 `DELETE /hardware/{id}`，不带 request body，不自动重放。官方 restore endpoint 说明 hardware delete 是 soft-delete；本模块不执行永久清除。
- delete HTTP success 后仍必须调用现有 Snipe-IT business-success envelope 检查；只有两层都成功才增加 `DeletedAssets`。一个 delete 失败增加 `FailedAssets`、产生 target-specific `ImportFailure` 并继续后续候选。
- 每个成功删除产生一条 `ActionType = Delete`、`TargetType = Asset`、`WasExecuted = true` action；dry-run 对应 action 的 `WasExecuted = false`。action identifier 使用十进制 Snipe-IT asset id，target name 使用 asset tag（缺失时退回设备名），message 固定包含 asset id、asset tag、设备名、Atera Agent ID 与 `MissingFromAtera` 原因。
- 成功和失败日志都使用结构化字段 `AssetId`、`AssetTag`、`AssetName`、`AteraAgentId`、`DeleteReason`、`DeleteResult`。日志 message template 不拼接 notes、token、request headers 或 raw response body。
- status history 的 `assetsDeleted` 必须来自 `SnipeImportResult.DeletedAssets`，`assets.deleted` 必须包含 delete actions；Worker terminal summary 的 `Deleted` 同样投影真实 count，不得再固定为 0。
- 必测安全边界：非 `ATERA-` 人工资产不删除；当前 tag 不删除；stale tag dry-run 不发 DELETE；真实 run 发送正确 id 的 DELETE；empty Atera batch 删除全部 Atera-managed assets 但保留人工资产；malformed/incomplete hardware snapshot 无 mutation；单个 delete failure 不阻塞后续 delete；日志和 history 包含关键审计字段且不包含 token/notes。

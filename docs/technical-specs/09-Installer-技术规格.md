...
现有 `Directory.Build.props` 提供统一 version/company metadata，并关闭 SDK 自动向 InformationalVersion 追加 source revision，确保 Windows ProductVersion 严格为 `1.0.0`；commit 继续由 release manifest 单独记录。Worker project 明确排除 development settings 的 publish。
...
- `FileVersionInfo` 验证 ProductVersion 严格为 `1.0.0`、FileVersion 为 `1.0.0.0`、CompanyName 为 `Vue IT Inc.`。

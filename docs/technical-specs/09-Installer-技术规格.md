...
现有 `Directory.Build.props` 提供统一 version/company metadata，并关闭 SDK 自动向 InformationalVersion 追加 source revision，确保 Windows ProductVersion 严格为 `1.0.1`；commit 继续由 release manifest 单独记录。Worker project 明确排除 development settings 的 publish。
...
- Project owner 已于 2026-07-23 明确同意适用的 WiX 7 OSMF EULA；project 使用 `<AcceptEula>wix7</AcceptEula>` 记录该授权，供 non-interactive release build 使用。
- `InstallerPlatform=x64`、`OutputType=Package`、`SuppressValidation=false`。
- 使用 bind path/MSBuild property `PublishRoot` 指向 merged staging root。
- build 必须收到 `Version`、`ProductCode`、`PublishRoot`、`ReleaseOutputRoot`；缺失时 fail fast。
...
10. 验证 MSI 存在，删除 release output 中不发布的 `.wixpdb`，写入 lowercase SHA-256 line 和 JSON manifest。
11. 通过 `msiexec /a` 行政解包，并自动验证两个 EXE 同目录、严格版本/公司元数据、Tray 图标、禁入文件、SHA-256 sidecar 和 manifest；任何检查失败时整个 release command 失败。
...
- `FileVersionInfo` 验证 ProductVersion 严格为 `1.0.1`、FileVersion 为 `1.0.1.0`、CompanyName 为 `Vue IT Inc.`。
...
完成 automated verification并更新 `progress.md` 后，审查 status，只 stage v1.0.1 files，commit message `release: prepare v1.0.1`。从 clean commit重建 final MSI。仅在两套 VM matrix通过后创建 annotated tag `v1.0.1`；不自动 push。

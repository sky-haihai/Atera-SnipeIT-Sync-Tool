...
## 2026-07-23 Preview 不占用 Latest run 回归

`LatestSyncHistoryReaderTests` 使用临时历史目录验证 Latest run 的真实同步选择规则：

- newest Preview (`run.dryRun = true`) 会被跳过，返回较旧但有效的 real sync counts/time；
- malformed newest JSON 不会遮住较旧的有效 real sync；
- 只有 Preview 的历史返回 null；
- 普通 count projection 与缺少 `assetsDeleted` 时默认为 0 的行为继续使用显式 `dryRun = false` fixture 覆盖。

`DashboardLayoutTests.PreviewResult_DoesNotReplaceLatestRun` 在离线 STA control tree 中先渲染一份 real-sync terminal summary，再渲染更新的 Preview summary；测试要求 Latest run 的 timestamp/count label 集合完全不变，同时 Current activity message 仍更新为 Preview 的 terminal outcome。测试只使用临时 config/log 和不可用的测试 pipe，不启动 Worker、SCM 或外部 API。
...
人工验收：先执行一次真实 Sync Now 并记录 Latest run 的时间和四项计数，再执行 Preview。Preview 的结果应出现在 Current activity 和日志中，但 Latest run 仍显示之前的真实 sync。重启 TrayApp，并在 Worker 离线时再打开 Dashboard，确认本地历史回退仍选择同一 real sync。不要为此测试打印或复制 Atera/Snipe-IT credentials。
